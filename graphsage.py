...
class UnsupervisedLoss(object):
    ...
    def __init__(self, adj_lists, train_nodes, device, config):
...
def apply_model(nodes, graphSage, classification, unsupervised_loss, lr, num_batches, batch_size):
    ...
    for index in range(batches):
        ...
        loss = unsupervised_loss.get_loss_sage(embs_batch, nodes_batch)
...
def run(config, G):
    ...
    adj = nx.convert.to_dict_of_lists(G)
    new_ind = {k: v for v, k in enumerate(list(adj.keys()))}
    adj = dict([(new_ind.get(key), value) for key, value in adj.items()])
    ...
    for epoch in range(config['iter']):
        print('----------------------EPOCH %d-----------------------' % epoch)
        graphSage, classification = apply_model(nodes, graphSage, classification, unsupervised_loss, lr=config['learning-rate'], num_batches=config['batch'], batch_size=config['batch-size'])

    emb = graphSage(nodes).tolist()
